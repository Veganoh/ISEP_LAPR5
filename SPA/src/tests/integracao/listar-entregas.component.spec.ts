...
describe('CriarArmazensComponente', () => {
    ...
    let listarEntregaService: ListarEntregasServico;
    let listarArmazemService: ListarArmazensServico;
    let ordenarEntregaService: OrdenarEntregasService;
    ...
    let httpElistarSpy:any = jasmine.createSpyObj('HttpClient', ['get']);
    let httpAlistarSpy:any = jasmine.createSpyObj('HttpClient', ['get']);
    let httpOlistarSpy:any = jasmine.createSpyObj('HttpClient', ['get']);

    beforeEach(() => {
        listarArmazemService = new ListarArmazensServico(httpAlistarSpy);
        listarEntregaService = new ListarEntregasServico(httpElistarSpy);
        ordenarEntregaService = new OrdenarEntregasService(httpOlistarSpy);

        httpAlistarSpy.get.and.returnValue(of(armazemLista));
        httpElistarSpy.get.and.returnValue(of(entregaLista));
        httpOlistarSpy.get.and.returnValue(of(entregaLista));
